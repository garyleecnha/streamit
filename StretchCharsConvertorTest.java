...
@SpringBootTest
public class StretchCharsConvertorTest {

	@Autowired
	private StretchCharsConvertor theService;
	...
	public void stageOneTest() {
		...
		List<String> list = stringItemStream().toList();
    		...
    		Assert.assertEquals( theService.stageProcessor(list.get(i),"stage#1"), list.get(i+1) );
	...
	public void stageTwoTest() {
		...
		List<String> list = strItemStream().toList();
        	Stream.iterate(0, i -> i + 2).limit(list.size()/2).forEach(i -> {
    		Assert.assertEquals( theService.stageProcessor(list.get(i),"stage#2"), list.get(i+1) );
        	System.out.println("<"+i/2+">: "+String.format("%" + 20 + "s", list.get(i))+"-->"+list.get(i+1));
        });	
	}
}
